...
if pages == 'Introduction and view Table':
    ...
    model_regression = artifacts['model_regression']
    ...
    selected_state = st.selectbox("Select a State", options=["Select State"] + state_list)

    if selected_state != "Select State":
        city_list = data[data['State'] == selected_state]['City'].unique().tolist()
        selected_city = st.selectbox("Select a City", options=["Select City"] + city_list)
    else:
        selected_city = st.selectbox("Select a City", options=[], disabled=True)

    if selected_city not in ["Select City", None]:
        locality_list = data[data['City'] == selected_city]['Locality'].unique().tolist()
        selected_locality = st.selectbox("Select a Locality", options=locality_list)
    else:
        selected_locality = st.selectbox("Select a Locality", options=[], disabled=True)
    ...
    if st.button("Predict Results"):
        ...
        for col in multilabel_cols:
            ...
            # Check if the value is None or empty; if so, pass an empty list
            if val is None:
                clean_val = []
            elif isinstance(val, str):
                clean_val = [val.split(',')]
            else:
                clean_val = [[val]] # Handle case if it's already a list/object

            temp = mlb.transform(clean_val)
        ...
        # 4. EXECUTE BOTH PREDICTIONS
        res_classification = model.predict(X_final.values)
        res_regression = model_regression.predict(X_final.values)
        ...
        with c1:
            st.metric("Category / Classification", str(res_classification[0]))
        
        with c2:
            # Assuming regression is predicting total price
            st.metric("Estimated Market Value", f"₹ {res_regression[0]:,.2f}")
